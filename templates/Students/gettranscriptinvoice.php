...
<!--                                 <li>City: <span>London E1 8BF</span></li>
                    ...
                    <div>
                        ...
                        <div class="invoice-info">
                            ...
                            <p class="text-muted">Please click on either the yellow button below to make payment. All applications without fee payment will not be processed.<br />
...
// echo $service_charge = $transaction->charge; 20*1000/100
 ...
 //split and give netpro 70 then give uaes 30

  $split_amount =    $transaction->amount-300;
 $netpro_split = round((0.7*$split_amount),2);
 $uaes_split = round((0.3*$split_amount),2); 
...
<form name="form1" action="<?=GATEWAYURL?>" method="post">
    <input name="product_id" type="hidden" value="<?=PRODUCT_ID?>" />
    <input name="pay_item_id" type="hidden" value="<?=PAYMENT_ITEM_ID ?>" />
...
$string1  = $payref. PRODUCT_ID.PAYMENT_ITEM_ID. $amount_to_pass.$return_url.MACKEY; 
  ...
  $xml = Cake\Utility\Xml::build('<payment_item_detail>
        <item_details detail_ref="' . $payref . '" college="UAES" department="ITServices" faculty="Engineering"> 
            ...
            <item_detail item_id="2" item_name="Application fees2" item_amt="' . $uaes_split*100 . '" bank_id="8" acct_num="2031912457" />
        </item_details> 
    </payment_item_detail>');
  //echo $xml->asXML(); 
//  
...
<!--                                     <button class="btn btn-white" onclick="printDiv('printableArea')" ><i class="fa fa-print fa-lg"></i> Print</button>     -->
                                         <?php } else{  
                                             echo $this->Form->button('Pay Online(Interswitch)',['class'=>'btn btn-primary pull-right','title'=>'pay online with InterSwitch']) ;
                                         } 
